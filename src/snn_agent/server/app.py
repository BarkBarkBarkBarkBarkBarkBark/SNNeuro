...
def main() -> None:
    ...
    if args.config:
        ...
        optimized = _load_optimized_config(explicit_path)
        if optimized is None:
            print(f"⚠  Could not load --config {args.config} — using built-in defaults")
        else:
    ...
    if optimized is not None:
        ...
        cfg = DEFAULT_CONFIG
        if not args.no_optimized and not args.config:
